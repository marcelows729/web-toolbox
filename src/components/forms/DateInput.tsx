...
import { sanitizeDateTextValue } from '../../utils/dateInputValidation.ts'
...
export default function DateInput({
  ...
  const handleTextChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const nextValue = sanitizeDateTextValue(event.target.value)
    onChange(nextValue)
  }
  ...
  const handlePaste = (event: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    if (yearPart.length > 4 || parts.some((part) => part.length > 2 && part !== '')) {
      event.preventDefault()
      return
    }

    if (!/^\d{1,4}(?:-\d{0,2}){0,2}-?$/.test(cleaned)) {
      event.preventDefault()
      return
    }
  }
